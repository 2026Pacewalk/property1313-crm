    ...
    <>
      <nav className="fixed bottom-0 left-0 right-0 h-14 bg-p13-black border-t border-white/[0.08] z-50 flex items-center justify-around px-2">
        ...
        <button
          ...
          className={cn(
            'flex flex-col items-center justify-center gap-0.5 py-1 px-3 rounded-lg transition-colors',
            showMore ? 'text-p13-yellow' : 'text-neutral-400'
          )}
        >
      ...
      {showMore && (
        <>
          ...
          <div className="fixed bottom-16 left-2 right-2 bg-white dark:bg-[#1E293B] rounded-xl shadow-2xl border border-neutral-200 dark:border-slate-700 z-50 overflow-hidden">
            {isAdmin && (
              <button onClick={() => { navigate('/admin/users'); setShowMore(false); }}
                className="flex items-center gap-3 w-full px-4 py-3 hover:bg-neutral-50 dark:hover:bg-white/5 transition-colors text-left">
                <Shield size={16} className="text-p13-yellow" />
                <span className="text-sm font-medium dark:text-white">Users</span>
              </button>
            )}
            {isAdmin && (
              <button onClick={() => { navigate('/master-database'); setShowMore(false); }}
                className="flex items-center gap-3 w-full px-4 py-3 hover:bg-neutral-50 dark:hover:bg-white/5 transition-colors text-left border-t border-neutral-100 dark:border-slate-700">
                <Settings size={16} className="text-p13-yellow" />
                <span className="text-sm font-medium dark:text-white">Master Database</span>
              </button>
            )}
            ...
            <button onClick={logout}
              className="flex items-center gap-3 w-full px-4 py-3 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors text-left border-t border-neutral-100 dark:border-slate-700">

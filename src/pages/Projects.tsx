    ...
    <div className="page-container pt-4 pb-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-caption text-neutral-400">{projects.length} Projects</span>
        </div>
      ...
      <div className="relative mb-4">
        ...
        <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search projects..." className="pl-9 h-10 bg-white" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map((project, i) => (
          <motion.div key={project.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.06, 0.5) }}
            className="bg-white rounded-xl shadow-xs border border-neutral-200/50 overflow-hidden hover:shadow-md transition-all hover:-translate-y-0.5 cursor-pointer group">
            {/* Media */}
            <div className="relative h-44 md:h-52 overflow-hidden">
              ...
              <div className="absolute top-3 left-3 flex gap-1.5">
                <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full ${project.status === 'active' ? 'bg-green-500 text-white' : 'bg-neutral-500 text-white'}`}>
                  {project.status === 'active' ? 'Active' : 'Inactive'}
                </span>
              </div>
              <div className="absolute top-3 right-3 flex gap-1.5">
                <button onClick={() => { navigator.clipboard.writeText(project.shareLink); addToast({ type: 'success', message: 'Link copied!' }); }}
                  className="w-8 h-8 bg-black/40 backdrop-blur rounded-full flex items-center justify-center text-white hover:bg-black/60">
                  <Share2 size={14} />
                </button>
            ...
            <div className="p-3">
              ...
              <p className="text-xs text-neutral-400 mb-3">{project.propertyType.join(', ')}</p>
              <div className="flex items-center justify-between">
                <div className="flex gap-3">
                  <span className="text-[11px] text-neutral-400 flex items-center gap-1"><Users size={12} />{project.leadCount} Leads</span>
                  <span className="text-[11px] text-neutral-400 flex items-center gap-1"><Eye size={12} />{project.viewCount >= 1000 ? `${(project.viewCount / 1000).toFixed(1)}k` : project.viewCount} views</span>
                </div>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12">
          <Building2 size={48} className="text-neutral-300 mx-auto mb-3" />
          ...
          return (
            <div className="space-y-1 py-2">
              <button onClick={() => { navigate(`/project/${ap.slug}`); setShowActions(false); }}
                ...
                <div><p className="text-sm font-medium">View Public Page</p><p className="text-[11px] text-neutral-400">Open project public listing</p></div>
              </button>
              {can('manage_projects') && (
                <button onClick={() => { navigate(`/projects/${ap.slug}/edit`); setShowActions(false); }}
                  className="flex items-center gap-3 w-full px-3 py-3 rounded-lg hover:bg-neutral-100 transition-colors text-left">
                  <Pencil size={16} className="text-p13-yellow" />
                  <div><p className="text-sm font-medium">Edit Project</p><p className="text-[11px] text-neutral-400">Modify project details</p></div>
                </button>
              )}
              <button onClick={() => { navigator.clipboard.writeText(ap.shareLink); addToast({ type: 'success', message: 'Link copied!' }); setShowActions(false); }}
                ...
                <div><p className="text-sm font-medium">Copy Share Link</p><p className="text-[11px] text-neutral-400">Copy to clipboard</p></div>
              </button>
              {user?.role === 'super_admin' && (
                <button onClick={() => { if (confirm(`Delete "${ap.name}"?`)) { setShowActions(false); addToast({ type: 'success', message: `${ap.name} deleted` }); } }}
                  ...
                  <div><p className="text-sm font-medium text-red-600">Delete Project</p><p className="text-[11px] text-neutral-400">Permanently remove</p></div>
                </button>
              )}
            </div>
          );
      ...
      <BottomSheet isOpen={showAdd} onClose={() => setShowAdd(false)} title="Add Project">
        <div className="space-y-4 py-2">
          {['Project Name', 'Location', 'Description'].map(label => (
            <div key={label}><label className="text-xs font-medium text-neutral-500 mb-1 block">{label}</label>
              {label === 'Description' ? (
          ...
          <div><label className="text-xs font-medium text-neutral-500 mb-1 block">Property Type</label>
            <div className="flex flex-wrap gap-2">
              {['Apartment', 'Villa', 'Plot', 'Commercial'].map(t => (
                <button key={t} className="px-3 py-1.5 rounded-full text-xs font-medium bg-neutral-200 text-neutral-500">{t}</button>
              ))}
            </div>
          </div>
          <div><label className="text-xs font-medium text-neutral-500 mb-1 block">Cover Image</label>
            <div className="border-2 border-dashed border-neutral-300 rounded-lg h-24 flex items-center justify-center">
              <span className="text-xs text-neutral-400">Drag & drop or click to upload</span>
            </div>
          </div>

...
function Section({ title, icon, children, defaultOpen = true }: {
  ...
  return (
    <div className="border border-neutral-200 rounded-lg bg-white overflow-hidden">
      ...
      {open && <div className="px-4 pb-4 border-t border-neutral-100 pt-3 space-y-4">{children}</div>}
    </div>
  );
}
...
function TextInput({ value, onChange, placeholder, type = 'text' }: {
  ...
  return (
    <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
      className="w-full h-11 px-3 rounded-lg border border-neutral-200 bg-white text-sm focus:border-p13-yellow focus:ring-2 focus:ring-p13-yellow/20 outline-none transition-all" />
  );
}
...
function TextArea({ value, onChange, placeholder, rows = 3 }: {
  ...
  return (
    <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={rows}
      className="w-full px-3 py-2.5 rounded-lg border border-neutral-200 bg-white text-sm resize-none focus:border-p13-yellow focus:ring-2 focus:ring-p13-yellow/20 outline-none transition-all" />
  );
}
  ...
  return (
    <div className="relative">
      <select value={value} onChange={e => onChange(e.target.value)}
        className="w-full h-11 px-3 pr-10 rounded-lg border border-neutral-200 bg-white text-sm appearance-none focus:border-p13-yellow focus:ring-2 focus:ring-p13-yellow/20 outline-none transition-all">
    ...
    <div ref={ref} className="relative">
      <div className="relative">
        <input
          ...
          className="w-full h-11 px-3 pr-10 rounded-lg border border-neutral-200 bg-white text-sm focus:border-p13-yellow focus:ring-2 focus:ring-p13-yellow/20 outline-none transition-all"
        />
        <button
          onClick={() => setOpen(!open)}
          className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 flex items-center justify-center text-neutral-400 hover:text-neutral-600"
        >
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute z-50 left-0 right-0 mt-1 bg-white border border-neutral-200 rounded-lg shadow-lg max-h-52 overflow-y-auto">
            ...
            <div className="py-1">
              {filtered.map(opt => (
                <button
                  ...
                  className={cn('w-full text-left px-3 py-2 text-sm hover:bg-neutral-50 transition-colors flex items-center gap-2',
          ...
          {filtered.length === 0 && !showCustom && (
            <div className="px-3 py-4 text-center text-xs text-neutral-400">
              <Search size={14} className="mx-auto mb-1 text-neutral-300" />
...
export default function EditProject() {
  ...
  if (!user || !can('manage_projects')) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-sm text-neutral-500 mt-1">You do not have permission to edit projects.</p>
  ...
  if (!project) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        <Building2 size={48} className="text-neutral-300 mb-4" />
        <h2 className="text-lg font-semibold">Project Not Found</h2>
        <p className="text-sm text-neutral-500 mt-1">The project you are trying to edit does not exist.</p>
        <button onClick={() => navigate('/projects')} className="mt-4 h-10 px-6 bg-p13-yellow text-p13-black rounded-lg text-sm font-medium">
          Back to Projects
        </button>
      </div>
    ...
    <div className="min-h-[100dvh] bg-neutral-50 flex flex-col">
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-neutral-200">
        <div className="flex items-center gap-3 px-4 h-14">
          <button onClick={() => navigate(-1)} className="text-neutral-600 hover:text-neutral-900 p-1 -ml-1">
            <ArrowLeft size={20} />
          </button>
          <div className="flex-1 min-w-0">
            <h1 className="text-[15px] font-semibold truncate">Edit Project</h1>
            <p className="text-[11px] text-neutral-400 truncate">{project.name}</p>
          </div>
        ...
        <div className="max-w-2xl mx-auto px-4 py-4 space-y-4 pb-28">
              ...
              <div>
                ...
                <div className="relative">
                  <select value={status} onChange={e => setStatus(e.target.value as 'active' | 'inactive')}
                    className="w-full h-11 px-3 pr-10 rounded-lg border border-neutral-200 bg-white text-sm appearance-none focus:border-p13-yellow outline-none">
              ...
              {coverImage ? (
                <div className="relative rounded-lg overflow-hidden border border-neutral-200">
                  ...
                  <div className="absolute top-2 right-2 flex gap-1.5">
                    <label className="h-8 px-3 bg-black/60 backdrop-blur text-white rounded-lg text-xs font-medium flex items-center gap-1 cursor-pointer hover:bg-black/80">
                    ...
                    <button onClick={() => setCoverImage('')}
                      className="h-8 w-8 bg-red-500/80 backdrop-blur text-white rounded-lg flex items-center justify-center hover:bg-red-600">
                      <Trash2 size={12} />
                    </button>
                  </div>
                </div>
                ...
                <label className="border-2 border-dashed border-neutral-300 rounded-lg h-32 flex flex-col items-center justify-center cursor-pointer hover:border-p13-yellow hover:bg-p13-yellow/5 transition-all">
                  ...
                  <span className="text-xs text-neutral-400">Click to upload cover image</span>
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                    const file = e.target.files?.[0];
                    if (file) {
                      const reader = new FileReader();
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                  ...
                  <div key={i} className="relative aspect-square rounded-lg overflow-hidden border border-neutral-200">
                    ...
                    {showGalleryManage && (
                      <button onClick={() => handleRemoveGalleryImage(i)}
                        className="absolute top-1 right-1 w-6 h-6 bg-red-500/80 text-white rounded-full flex items-center justify-center hover:bg-red-600">
                        <X size={12} />
                      </button>
                    )}
                  </div>
                ...
                <button onClick={handleAddGalleryImage}
                  ...
                  <span className="text-[10px] text-neutral-400 mt-0.5">Add</span>
                </button>
              </div>
            </div>
          ...
          <Section title="Public Listing" icon={<ExternalLink size={16} className="text-p13-yellow" />}>
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <p className="text-[11px] text-neutral-400">Make this project visible on the public site</p>
              </div>
              <button onClick={() => setPublicEnabled(!publicEnabled)}
                ...
                <div className={cn('w-5 h-5 bg-white rounded-full absolute top-1 transition-all shadow-sm', publicEnabled ? 'left-6' : 'left-1')} />
              </button>
            </div>
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={handleRegenerateSlug}
                  className="h-11 px-3 bg-neutral-100 text-neutral-600 rounded-lg text-xs font-medium whitespace-nowrap hover:bg-neutral-200 transition-all">
                  Regenerate
                </button>
              </div>
              <p className="text-[10px] text-neutral-400 mt-1">URL: {publicUrl}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={handleCopyLink}
                className="flex-1 h-10 bg-neutral-100 text-neutral-700 rounded-lg text-xs font-medium flex items-center justify-center gap-1.5 hover:bg-neutral-200 transition-all">
          ...
          <Section title="Description & Highlights" icon={<Home size={16} className="text-p13-yellow" />}>
            <div>
              ...
              <p className="text-[10px] text-neutral-400 mt-1">{shortDesc.length}/120 characters</p>
            </div>
            <div>
      ...
      <div className="sticky bottom-0 z-50 bg-white border-t border-neutral-200 px-4 py-3 md:hidden">
        <button onClick={handleSave} disabled={saving}

...
import { isValidMobile, normalizeMobile } from '@/lib/phone-validation';
import { cn } from '@/lib/utils';

export default function Login() {
  const navigate = useNavigate();
  const { login, loginWithOtp, requestPasswordReset, completeForcedPasswordChange, checkStoredSession, getRedirectPath, setRememberMe, isAccountLocked, getLockoutTimeRemaining, isIdentifierEmail } = useAuthStore();
  ...
  const detected = !identifier ? null : isIdentifierEmail(identifier.trim()) ? 'email' : isValidMobile(identifier.trim()) ? 'mobile' : null;
  ...
  // Forced password change (first login): completes auth only after a valid new password
  const handleForcedPasswordChange = async () => {
    setError('');
    const result = completeForcedPasswordChange(newPass, confirmPass);
    if (!result.success) { setError(result.error || 'Could not update password'); return; }
    addToast({ type: 'success', message: 'Password updated! Welcome.' });
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4 py-8 relative">
      ...
      <div className="relative w-full max-w-[420px]">
        ...
        <AnimatePresence mode="wait">
          ...
          {screen === 'login' && (
            <motion.div key="login" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              {/* Tabs */}
              <div className="flex bg-card rounded-lg p-1 mb-6 border border-border shadow-sm">
                {[
                  ...
                  <button key={t.key} onClick={() => { setTab(t.key); setError(''); setOtpSent(false); setOtp(['', '', '', '', '', '']); }}
                    className={cn('flex-1 flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-md text-xs font-medium transition-all', tab === t.key ? 'bg-p13-yellow text-p13-black' : 'text-muted-foreground hover:text-foreground')}>
              ...
              <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
                {/* Identifier Input */}
                <div className="mb-4">
                  <label className="text-[11px] font-medium text-muted-foreground mb-1.5 block">Email or Mobile Number</label>
                  <div className="relative">
                    {detected === 'email' ? <Mail size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /> :
                     detected === 'mobile' ? <Phone size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /> :
                    ...
                    <input value={identifier} onChange={(e) => { setIdentifier(e.target.value); setError(''); }} placeholder="Enter email or mobile"
                      className={cn('w-full h-11 pl-10 pr-3 rounded-lg bg-background border text-foreground text-base placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-p13-yellow/20 transition-all',
                        detected === 'email' ? 'border-blue-300' : detected === 'mobile' ? 'border-green-300' : 'border-border', 'focus:border-p13-yellow')} />
                  </div>
                ...
                <AnimatePresence mode="wait">
                      ...
                      <div className="mb-4">
                        ...
                        <div className="relative">
                          ...
                          <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => { setPassword(e.target.value); setError(''); }} placeholder="Enter password"
                            className="w-full h-11 pl-10 pr-10 rounded-lg bg-background border border-border text-foreground text-base placeholder:text-muted-foreground focus:outline-none focus:border-p13-yellow focus:ring-2 focus:ring-p13-yellow/20 transition-all" />
                          <button onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-gray-600">
                  ...
                  {tab === 'otp' && (
                    <motion.div key="otp" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      {!otpSent ? (
                        <div>
                          <p className="text-xs text-muted-foreground mb-4">We will send a 6-digit OTP to your registered WhatsApp number.</p>
                        ...
                        <div>
                          <p className="text-xs text-muted-foreground mb-3">Enter the 6-digit code sent to your WhatsApp</p>
                          <div className="flex gap-2 justify-center mb-4" onPaste={handleOtpPaste}>
                            {otp.map((digit, i) => (
                              <input key={i} ref={(el) => { otpRefs.current[i] = el; }} type="text" inputMode="numeric" maxLength={1} value={digit}
                                ...
                                className="w-11 h-12 bg-background border border-border rounded-lg text-center text-xl font-semibold text-foreground focus:border-p13-yellow focus:ring-2 focus:ring-p13-yellow/20 focus:bg-yellow-50 outline-none transition-all" />
                            ))}
                          </div>
          ...
          {screen === 'forgot' && (
            <motion.div key="forgot" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
                <button onClick={() => setScreen('login')} className="flex items-center gap-1 text-muted-foreground hover:text-foreground text-xs mb-4">
                ...
                <AnimatePresence mode="wait">
                  {resetStep === 'request' && (
                    <motion.div key="req" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      <div className="text-center mb-5">
                        ...
                        <h2 className="text-foreground text-lg font-semibold">Reset Password</h2>
                        <p className="text-muted-foreground text-xs mt-1">Enter your email or mobile to receive a reset link</p>
                      </div>
                      <div className="mb-4">
                        <label className="text-[11px] font-medium text-muted-foreground mb-1.5 block">Email or Mobile Number</label>
                        <div className="relative">
                          {isIdentifierEmail(forgotIdentifier) ? <Mail size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /> :
                          ...
                          <input value={forgotIdentifier} onChange={(e) => { setForgotIdentifier(e.target.value); setError(''); }} placeholder="Enter email or mobile"
                            className="w-full h-11 pl-10 pr-3 rounded-lg bg-background border border-border text-foreground text-base placeholder:text-muted-foreground focus:outline-none focus:border-p13-yellow focus:ring-2 focus:ring-p13-yellow/20 transition-all" />
                        </div>
                      </div>
                      {error && <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-lg p-3 mb-4"><AlertCircle size={14} className="text-red-500" /><p className="text-xs text-red-600">{error}</p></div>}
                  ...
                  {resetStep === 'sent' && (
                    <motion.div key="sent" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-4">
                      ...
                      <h3 className="text-foreground text-base font-semibold mb-1">Check your inbox</h3>
                      ...
                      <button onClick={() => { setScreen('login'); setResetStep('request'); }} className="text-xs text-muted-foreground hover:text-foreground mt-2">
                        Back to login
                      </button>
                    </motion.div>
                  )}
                  ...
                  {resetStep === 'form' && (
                    <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      <h2 className="text-foreground text-lg font-semibold mb-1">Set New Password</h2>
                      <p className="text-muted-foreground text-xs mb-4">Create a strong password for your account</p>

                      <div className="mb-3">
                        <label className="text-[11px] font-medium text-muted-foreground mb-1.5 block">New Password</label>
                        <div className="relative">
                          <input type={showNew ? 'text' : 'password'} value={newPass} onChange={(e) => { setNewPass(e.target.value); setError(''); }} placeholder="Min 8 chars, uppercase, number, special"
                            className="w-full h-11 pl-3 pr-10 rounded-lg bg-background border border-border text-foreground text-base placeholder:text-muted-foreground focus:outline-none focus:border-p13-yellow focus:ring-2 focus:ring-p13-yellow/20 transition-all" />
                          <button onClick={() => setShowNew(!showNew)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-gray-600">
                            {showNew ? <EyeOff size={15} /> : <Eye size={15} />}
                          </button>
                        </div>
                        {newPass && (
                          <div className="mt-2">
                            <div className="flex gap-1 mb-1">{[1, 2, 3, 4].map(i => <div key={i} className={cn('h-1 flex-1 rounded-full', i <= s ? strengthColors[s - 1] : 'bg-muted')} />)}</div>
                            <p className="text-[10px] text-muted-foreground">{strengthLabels[s - 1]}</p>
                          </div>
                        )}
                        <div className="mt-2 space-y-1">
                          {[{ label: '8+ characters', valid: newPass.length >= 8 }, { label: 'One uppercase letter', valid: /[A-Z]/.test(newPass) }, { label: 'One number', valid: /[0-9]/.test(newPass) }, { label: 'One special character', valid: /[!@#$%^&*]/.test(newPass) }].map((req) => (
                            <div key={req.label} className="flex items-center gap-1.5">
                              <div className={cn('w-3.5 h-3.5 rounded-full flex items-center justify-center', req.valid ? 'bg-green-500' : 'bg-muted')}>
                                {req.valid && <CheckCircle size={8} className="text-foreground" />}
                              </div>
                              <span className={cn('text-[11px]', req.valid ? 'text-green-600' : 'text-muted-foreground')}>{req.label}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="relative">
                          <input type={showConfirm ? 'text' : 'password'} value={confirmPass} onChange={(e) => { setConfirmPass(e.target.value); setError(''); }} placeholder="Confirm your password"
                            className="w-full h-11 pl-3 pr-10 rounded-lg bg-background border border-border text-foreground text-base placeholder:text-muted-foreground focus:outline-none focus:border-p13-yellow focus:ring-2 focus:ring-p13-yellow/20 transition-all" />
                          <button onClick={() => setShowConfirm(!showConfirm)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-gray-600">
          ...
          {screen === 'reset' && (
            <motion.div key="reset" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <div className="bg-card border border-border rounded-xl p-6 shadow-sm text-center">
                ...
                <h2 className="text-foreground text-lg font-semibold mb-1">Password Change Required</h2>
                <p className="text-muted-foreground text-xs mb-5">Your account requires a password update before you can continue.</p>
                <div className="text-left space-y-3">
                  <div>
                    ...
                    <input type="password" value={newPass} onChange={(e) => setNewPass(e.target.value)} placeholder="Min 8 chars, uppercase, number, special"
                      className="w-full h-11 px-3 rounded-lg bg-background border border-border text-foreground text-base placeholder:text-muted-foreground focus:outline-none focus:border-p13-yellow focus:ring-2 focus:ring-p13-yellow/20 transition-all" />
                  </div>
                  <div>
                    <label className="text-[11px] font-medium text-muted-foreground mb-1.5 block">Confirm Password</label>
                  ...
                  <button onClick={handleForcedPasswordChange} className="w-full h-11 bg-p13-yellow text-p13-black rounded-lg text-sm font-semibold hover:bg-p13-yellow/90">
        ...
        <p className="text-center text-[11px] text-muted-foreground mt-8">2026 Property1313. All rights reserved.</p>
      </div>
    </div>
  );
}

        ...
        <div className="px-4 py-3 border-b border-neutral-200 bg-white">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-neutral-500">{templates.filter(t => t.status === 'active').length} active templates</p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => setShowFilters(!showFilters)}
                className={cn('h-9 px-3 rounded-lg border text-xs font-medium flex items-center gap-1.5 transition-all',
                  showFilters ? 'bg-p13-yellow border-p13-yellow text-p13-black' : 'bg-white border-neutral-200 text-neutral-600')}
              >
                <Filter size={13} /> Filters
              </button>
              <button
                onClick={() => openEditor()}
                className="h-9 px-3 bg-p13-black text-white rounded-lg text-xs font-medium flex items-center gap-1.5 hover:bg-neutral-800 transition-all"
              >
                <Plus size={13} /> Create
              </button>
            </div>
          ...
          <div className="relative mb-2">
            ...
            />
            {filter.search && (
              <button onClick={() => setFilter({ search: '' })} className="absolute right-3 top-1/2 -translate-y-1/2 text-neutral-400">
                <X size={14} />
              </button>
            )}
          </div>
                ...
                <div className="pt-2 space-y-2">
                  <div className="flex gap-2 flex-wrap">
                    <select
                      ...
                      className="h-8 px-2 rounded-lg border border-neutral-200 bg-white text-xs outline-none focus:border-p13-yellow"
                    >
            ...
            {allCategories.slice(0, 10).map((c) => (
              <button
                ...
                className={cn('flex-shrink-0 px-2.5 py-1 rounded-full text-[11px] font-medium transition-all border',
                  filter.category === c.id ? 'bg-p13-yellow text-p13-black border-p13-yellow' : 'bg-white text-neutral-600 border-neutral-200')}
              >
                {c.name}
              </button>
            ))}
        ...
        <div className="flex-1 overflow-y-auto">
          {sorted.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-neutral-500 mb-1">No templates found</p>
      ...
      <AnimatePresence>
        {selectedTemplate && (
          <motion.div
            ...
            className={cn('flex-1 min-w-0 bg-white border-l border-neutral-200 flex flex-col',
              'fixed inset-0 z-40 md:static md:z-auto')}
          >
            {/* Mobile back button */}
            <div className="md:hidden flex items-center gap-2 px-4 py-3 border-b border-neutral-200">
              <button onClick={() => setSelectedId(null)} className="text-neutral-600">
                <X size={18} />
              </button>
            ...
            <div className="px-4 py-3 border-b border-neutral-200">
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-1">
                  <button onClick={() => toggleFavorite(selectedTemplate.id)} className={cn('p-1.5 rounded-lg', selectedTemplate.isFavorite ? 'text-p13-yellow' : 'text-neutral-400')}>
                    <Star size={15} fill={selectedTemplate.isFavorite ? 'currentColor' : 'none'} />
                  </button>
                  {isAdmin && (
                    <>
                      <button onClick={() => handleClone(selectedTemplate.id)} className="p-1.5 text-neutral-400 hover:text-neutral-600 rounded-lg">
                        <Copy size={15} />
                      </button>
                      <button onClick={() => openEditor(selectedTemplate.id)} className="p-1.5 text-neutral-400 hover:text-neutral-600 rounded-lg">
                        <Pencil size={15} />
                      </button>
                      <button onClick={() => handleArchive(selectedTemplate.id)} className="p-1.5 text-neutral-400 hover:text-neutral-600 rounded-lg">
                        <Archive size={15} />
                      </button>
            ...
            <div className="flex-1 overflow-y-auto">
              {/* WhatsApp Preview */}
              <div className="p-4">
                <p className="text-[11px] font-semibold text-neutral-500 mb-2 uppercase tracking-wider">WhatsApp Preview</p>
              ...
              {selectedTemplate.variables.length > 0 && (
                <div className="px-4 pb-4">
                  <p className="text-[11px] font-semibold text-neutral-500 mb-2 uppercase tracking-wider">Variables</p>
              ...
              {selectedTemplate.description && (
                <div className="px-4 pb-4">
                  <p className="text-[11px] font-semibold text-neutral-500 mb-1 uppercase tracking-wider">Description</p>
              ...
              {selectedTemplate.automationTriggers && selectedTemplate.automationTriggers.length > 0 && (
                <div className="px-4 pb-4">
                  <p className="text-[11px] font-semibold text-neutral-500 mb-2 uppercase tracking-wider">Automation Triggers</p>
                  <div className="flex flex-wrap gap-1.5">
              ...
              {selectedLogs.length > 0 && (
                <div className="px-4 pb-4">
                  <p className="text-[11px] font-semibold text-neutral-500 mb-2 uppercase tracking-wider">Recent Activity ({selectedLogs.length})</p>
                  <div className="space-y-2">
                    {selectedLogs.slice(0, 5).map((log) => (
                      <div key={log.id} className="bg-neutral-50 rounded-lg p-2.5 text-xs">
                        ...
                        <p className="text-neutral-500 text-[11px] line-clamp-2">{log.content}</p>
                        <div className="flex items-center justify-between mt-1 text-[10px] text-neutral-400">
                          <span>{new Date(log.createdAt).toLocaleDateString()}</span>
                          <span className={cn(log.sendMethod === 'automation' ? 'text-blue-500' : 'text-neutral-500')}>
...
function TemplateRow({
  ...
  return (
    <div
      className={cn('px-4 py-3 cursor-pointer transition-all hover:bg-neutral-50 group relative',
        isSelected ? 'bg-p13-yellow/5 border-l-[3px] border-l-p13-yellow' : 'border-l-[3px] border-l-transparent')}
      onClick={onClick}
    >
      <div className="flex items-start gap-3">
        {/* Message type indicator */}
        <div className="w-8 h-8 rounded-lg bg-neutral-100 flex items-center justify-center flex-shrink-0 text-xs">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-neutral-500 line-clamp-1 mb-1">{template.content.slice(0, 80)}...</p>
          <div className="flex items-center gap-2 text-[10px] text-neutral-400">
            ...
            <span className={cn('capitalize', template.status === 'active' ? 'text-green-500' : template.status === 'archived' ? 'text-neutral-400' : 'text-yellow-500')}>
              {template.status}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={(e) => { e.stopPropagation(); onToggleFavorite(); }} className={cn('p-1 rounded', template.isFavorite ? 'text-p13-yellow' : 'text-neutral-400')}>
            <Star size={13} fill={template.isFavorite ? 'currentColor' : 'none'} />
          </button>
          {isAdmin && (
            <>
              <button onClick={(e) => { e.stopPropagation(); onClone(); }} className="p-1 text-neutral-400 hover:text-neutral-600 rounded"><Copy size={13} /></button>
              <button onClick={(e) => { e.stopPropagation(); onArchive(); }} className="p-1 text-neutral-400 hover:text-neutral-600 rounded"><Archive size={13} /></button>

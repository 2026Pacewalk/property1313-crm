...
export default function LoanInquiry() {
  ...
  const statusBg: Record<string, string> = {
    new: 'bg-blue-100 text-blue-700', contacted: 'bg-yellow-100 text-yellow-700', 'in_progress': 'bg-purple-100 text-purple-700', approved: 'bg-green-100 text-green-700', closed: 'bg-neutral-100 text-neutral-600',
  };
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-2 mb-3 scrollbar-none">
        {tabs.map(t => (
          <button key={t} onClick={() => setActiveTab(t)}
            className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap capitalize transition-colors ${activeTab === t ? 'bg-p13-yellow text-p13-black' : 'bg-neutral-200 text-neutral-500'}`}>
      ...
      <div className="space-y-2">
        {filtered.map((inquiry, i) => (
          <motion.div key={inquiry.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
            className={`bg-white rounded-lg p-3.5 shadow-xs border border-neutral-200/50 ${statusColors[inquiry.status] || ''} border-l-[3px]`}>
            ...
            <div className="flex items-center gap-4 text-xs text-neutral-400 mb-1.5">
              <span>{inquiry.phone}</span>
              {inquiry.email && <span>{inquiry.email}</span>}
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                <IndianRupee size={12} className="text-neutral-500" />

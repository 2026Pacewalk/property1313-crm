  ...
  return (
    <div className="page-container pt-4 pb-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        ...
        className="bg-p13-black rounded-xl p-5 mb-5 cursor-pointer hover:bg-neutral-800 transition-colors">
        <div className="flex items-center gap-3">
          <div className="w-14 h-14 rounded-full border-2 border-p13-yellow flex items-center justify-center text-white text-lg font-bold flex-shrink-0"
      ...
      <div className="bg-white rounded-xl shadow-xs border border-neutral-200/50 mb-4">
        <button onClick={() => navigate('/profile')} className="flex items-center gap-3 w-full px-4 py-3 hover:bg-neutral-50 transition-colors text-left">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-neutral-400">View and manage your profile</p>
          </div>
      ...
      {settingsGroups.map((group: any, gi: number) => (
        <motion.div key={group.title} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: gi * 0.05 }}
          className="bg-white rounded-xl shadow-xs border border-neutral-200/50 mb-4 overflow-hidden">
          <div className="px-4 py-2.5 border-b border-neutral-100">
            <h3 className="text-sm font-semibold">{group.title}</h3>
          </div>
          {group.items.map((item: any, ii: number) => (
            <div key={ii} className={`flex items-center gap-3 px-4 py-3 ${ii < group.items.length - 1 ? 'border-b border-neutral-100' : ''} hover:bg-neutral-50 transition-colors`}>
              <item.icon size={16} className="text-neutral-400 flex-shrink-0" />
              <div className="flex-1 min-w-0">
              ...
              {item.control === 'toggle' && (
                <button className={`w-10 h-5 rounded-full transition-colors relative ${item.value ? 'bg-p13-yellow' : 'bg-neutral-300'}`}>
                  <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${item.value ? 'left-[22px]' : 'left-0.5'}`} />
                </button>
              )}
              {item.control === 'link' && (
                <button onClick={item.action}><ChevronRight size={16} className="text-neutral-400" /></button>
              )}
              {item.control === 'badge' && (
                <span className="text-[11px] px-2 py-0.5 bg-neutral-200 text-neutral-500 rounded-full">{item.value}</span>
              )}
              {item.control === 'button' && (
      ...
      {/* About */}
      <div className="bg-white rounded-xl shadow-xs border border-neutral-200/50 mb-6">
        <div className="px-4 py-3">
          ...
          <div className="flex items-center justify-between py-1">
            <span className="text-sm text-neutral-500">Build</span>
      ...
      <BottomSheet isOpen={showChangePass} onClose={() => setShowChangePass(false)} title="Change Password">
        <div className="space-y-4 py-2">
          <div><label className="text-xs font-medium text-neutral-500 mb-1 block">Current Password</label>
            <input type="password" className="w-full h-11 px-3 rounded-lg border border-neutral-200 bg-white text-sm" placeholder="Current password" /></div>
          <div><label className="text-xs font-medium text-neutral-500 mb-1 block">New Password</label>
            <input type="password" className="w-full h-11 px-3 rounded-lg border border-neutral-200 bg-white text-sm" placeholder="New password" /></div>
          <div><label className="text-xs font-medium text-neutral-500 mb-1 block">Confirm Password</label>
            <input type="password" className="w-full h-11 px-3 rounded-lg border border-neutral-200 bg-white text-sm" placeholder="Confirm password" /></div>

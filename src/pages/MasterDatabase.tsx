...
export default function MasterDatabase() {
  ...
  if (!user || !can('manage_projects')) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-sm text-muted-foreground mt-1">Only Super Admin and Admin can access Master Database.</p>
  ...
  return (
    <div className="h-[calc(100dvh-56px)] bg-muted/50 flex flex-col md:flex-row overflow-hidden">
      {/* ===== LEFT SIDEBAR: Master Types ===== */}
      <aside className="w-full md:w-72 bg-card border-r border-border flex flex-col flex-shrink-0">
        {/* Header */}
        <div className="px-4 py-3 border-b border-border">
          ...
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="w-full h-8 pl-8 pr-3 rounded-lg border border-border bg-card text-xs focus:border-p13-yellow outline-none" />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-1">
          {Object.entries(typesByModule).map(([module, types]) => (
            <div key={module}>
              <p className="px-4 py-1.5 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">{module}</p>
              {types.map(mt => {
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {/* Top Tabs */}
        <div className="bg-card border-b border-border px-4 flex items-center gap-1">
          {[
              ...
              <motion.div key="values" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground">{displayValues.length} active {displayValues.length === 1 ? 'value' : 'values'}{showInactive && inactiveValues.length > 0 ? ` (${inactiveValues.length} inactive)` : ''}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setShowInactive(!showInactive)}
                      className={cn('h-8 px-2.5 rounded-lg text-[11px] font-medium flex items-center gap-1.5 border transition-all',
                        showInactive ? 'bg-p13-yellow text-p13-black border-p13-yellow' : 'bg-card text-muted-foreground border-border')}>
                      {showInactive ? <Eye size={12} /> : <EyeOff size={12} />}
                      {showInactive ? 'Hide Inactive' : 'Show Inactive'}
                    </button>
                    <button onClick={() => setShowMergeSheet(true)}
                      className="h-8 px-2.5 bg-card text-muted-foreground border border-border rounded-lg text-[11px] font-medium flex items-center gap-1.5 hover:bg-muted/50">
                      <Merge size={12} /> Merge
                    </button>
                ...
                <div className="bg-card rounded-lg border border-border overflow-hidden">
                  {/* Header */}
                  <div className="grid grid-cols-[1fr_auto_auto_auto] md:grid-cols-[60px_1fr_120px_100px_auto] gap-2 px-4 py-2.5 bg-muted/50 border-b border-border text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                  ...
                  {displayValues.map((v, i) => {
                    ...
                    return (
                      <motion.div key={v.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }}
                        className={cn('grid grid-cols-[1fr_auto_auto_auto] md:grid-cols-[60px_1fr_120px_100px_auto] gap-2 px-4 py-2.5 border-b border-border/50 items-center',
                          !v.isActive && 'opacity-50')}>
                        {/* Order */}
                        <span className="hidden md:flex items-center gap-1">
                          <span className="text-[11px] text-muted-foreground w-5">{i + 1}</span>
                          <div className="flex flex-col">
                            <button onClick={() => moveUp(v)} className="text-muted-foreground/50 hover:text-muted-foreground"><ArrowUp size={10} /></button>
                            <button onClick={() => moveDown(v)} className="text-muted-foreground/50 hover:text-muted-foreground"><ArrowDown size={10} /></button>
                          </div>
                        </span>
                        ...
                        {/* Parent */}
                        <span className="hidden md:block text-[11px] text-muted-foreground truncate">{parentVal?.name || '-'}</span>
                        ...
                        <div className="flex items-center gap-0.5 justify-end">
                          <button onClick={() => startEdit(v)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-muted text-muted-foreground hover:text-muted-foreground" title="Edit">
                            <Pencil size={13} />
                          </button>
                          {v.isActive ? (
                            <button onClick={() => deactivateValue(v.id, user.id, user.name)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-muted text-green-400 hover:text-green-600" title="Deactivate">
                              <ToggleRight size={15} />
                            </button>
                          ) : (
                            <button onClick={() => activateValue(v.id, user.id, user.name)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-muted text-muted-foreground/50 hover:text-muted-foreground" title="Activate">
                              <ToggleLeft size={15} />
                            </button>
                          )}
                          <button onClick={() => setDeleteConfirm(v.id)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-red-50 text-muted-foreground hover:text-red-500" title="Delete">
                            <Trash2 size={13} />
                          </button>
                        </div>
                      </motion.div>
                  ...
                  {displayValues.length === 0 && (
                    <div className="py-10 text-center">
                      <FolderOpen size={28} className="text-muted-foreground/50 mx-auto mb-2" />
            ...
            {activeTab === 'fields' && (
              <motion.div key="fields" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <p className="text-[11px] text-muted-foreground">Control required, visible, and editable fields per module</p>
                </div>
                ...
                <div className="bg-card rounded-lg border border-border overflow-hidden">
                  <div className="grid grid-cols-[1fr_80px_80px_80px] gap-2 px-4 py-2.5 bg-muted/50 border-b border-border text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                    <span>Field</span>
                  ...
                  {fieldSettings.filter(fs => fs.moduleName === fieldModule).map(fs => (
                    <div key={fs.id} className="grid grid-cols-[1fr_80px_80px_80px] gap-2 px-4 py-2 border-b border-border/50 items-center">
                      <div>
                        ...
                        <p className="text-[10px] text-muted-foreground">{fs.fieldType} {fs.masterTypeId && <span className="text-p13-yellow"> (master)</span>}</p>
                      </div>
                ...
                <div className="space-y-2">
                  {auditLogs.length === 0 && (
                    <div className="text-center py-10">
                      <History size={28} className="text-muted-foreground/50 mx-auto mb-2" />
                      <p className="text-sm text-muted-foreground">No audit log entries yet</p>
                      <p className="text-[11px] text-muted-foreground mt-1">Actions will be logged as you manage master values</p>
                    </div>
                  )}
                  {auditLogs.map(log => {
                    const actionColors: Record<string, string> = {
                      ...
                      values_reordered: 'bg-muted text-muted-foreground',
                    };
                    return (
                      <div key={log.id} className="bg-card rounded-lg border border-border px-4 py-3 flex items-start gap-3">
                        <span className={cn('text-[10px] font-medium px-2 py-0.5 rounded-full flex-shrink-0 mt-0.5', actionColors[log.action] || 'bg-muted')}>
      ...
      <BottomSheet isOpen={showValueSheet} onClose={resetValueForm} title={editingValue ? 'Edit Value' : 'Add Value'} maxHeight="75vh">
        <div className="space-y-4 py-2">
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Value Name <span className="text-red-500">*</span></label>
          ...
          {selectedType?.hasParent && parentValues.length > 0 && (
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Parent ({parentType?.name})</label>
      ...
      <BottomSheet isOpen={showMergeSheet} onClose={() => { setShowMergeSheet(false); setMergeSource(''); setMergeTarget(''); }} title="Merge Values" maxHeight="60vh">
        <div className="space-y-4 py-2">
          <p className="text-xs text-muted-foreground">Select a source value to merge into a target. The source will be removed and its usage count transferred.</p>
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Source (will be removed)</label>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Target (will be kept)</label>
